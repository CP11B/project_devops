...
- In a terminal `git clone https://github.com/CP11B/project_devops`
- Navigate to the Terraform folder and perform the following commands.

**in main.tf you will need to change the ssh_key_name(line 40) to one you already have. It is suggested that you use aws-cli and run `aws ec2 create-key-pair --key-name [KEY_PAIR_NAME]`**

1. `terraform init` This initialises Terraform.

2. `terraform plan` This prints out all the changes that will be made if you run terraform apply.

3. `terraform apply` This applies those changes, and builds the Terraform infrastructure. Remember to type "yes" when prompted. 
